...
class AwayPlayersTab extends StatelessWidget {
  final MatchDetailEntity match;
  const AwayPlayersTab({super.key, required this.match});
          ...
          child: Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Danh sách cầu thủ ${match.awayTeamName ?? "Đội khách"}',
                    style: AppStyle.headline5,
                  ),
                  SizedBox(height: 16.sp),
                  if (state.status == MatchDetailStatus.loadingPlayers)
                    const AppLoading()
                  else if (state.awayTeamPlayers == null ||
                      state.awayTeamPlayers!.isEmpty)
                    EmptyWidget(
                      message: 'Chưa có dữ liệu cầu thủ',
                      description:
                          'Danh sách cầu thủ đội khách chưa được cập nhật',
                      buttonText: 'Tự động thêm cầu thủ',
                      onButtonPressed: () {
                        context
                            .read<MatchDetailCubit>()
                            .autoRegisterAwayPlayersForMatch();
                      },
                    )
                  else
                    MatchPlayerListview(players: state.awayTeamPlayers!),
                ],
              ),
            ),
          ),
